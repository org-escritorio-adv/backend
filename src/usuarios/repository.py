...
REALM_NAME = os.getenv("KEYCLOAK_REALM", "escritorio-adv")
CLIENT_ID = os.getenv("KEYCLOAK_ADMIN_CLIENT_ID", "backend-client")
...
def criar(db=None, dados=None):
    ...
    response = requests.post(
        ...
        json={
            ...
            "lastName": last_name,
            "enabled": True,
            "emailVerified": True,          # evita bloqueio de login por verificação de e-mail
            "requiredActions": [],          # garante que nenhuma ação obrigatória bloqueie o login
    ...
    if response.status_code not in (200, 201):
        raise HTTPException(status_code=500, detail="Erro ao criar usuário no Keycloak.")
    ...
    if role_resp.status_code != 200:
        raise HTTPException(
            status_code=500,
            detail=(
                f"Role '{keycloak_role}' não encontrada no Keycloak. "
                "Verifique se a role existe e se o service account do backend tem permissão 'manage-users'."
            ),
        )
    ...
    if assign_resp.status_code not in (200, 204):
        raise HTTPException(
            status_code=500,
            detail=(
                f"Usuário criado mas falha ao atribuir role '{keycloak_role}'. "
                "O service account do backend precisa do role 'manage-users' em realm-management."
            ),
        )

    avatar = "".join(p[0] for p in dados.nome.split()[:2]).upper()
    return {
        "id": user_id,
        "nome": dados.nome,
        "email": dados.email,
